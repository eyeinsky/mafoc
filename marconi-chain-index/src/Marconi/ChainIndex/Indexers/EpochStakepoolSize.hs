...
sqliteInit c tableName = SQL.execute_ c $
  "CREATE TABLE IF NOT EXISTS " <> fromString tableName <> " (poolId BLOB NOT NULL, lovelace INT NOT NULL, epochNo INT NOT NULL)"

sqliteInsert :: SQL.Connection -> String -> [Event] -> IO ()
sqliteInsert c tableName events = SQL.executeMany c
  ("INSERT INTO " <> fromString tableName <>" (epochNo, poolId, lovelace) VALUES (?, ?, ?)")
  (toRows =<< events)
  where
...
sqlite c source = lift (sqliteInit c tableName) >> loop source
  where
    loop :: S.Stream (S.Of Event) IO r -> S.Stream (S.Of Event) IO r
    loop source' = lift (S.next source') >>= \case
        Left r -> pure r
        Right (event, source'') -> do
          lift $ sqliteInsert c tableName [event]
          S.yield event
          loop source''
    tableName :: String
    tableName = "stakepool_delegation"
